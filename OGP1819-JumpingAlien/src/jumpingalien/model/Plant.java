...
public class Plant extends GameObject {
	...
	public void setBoundaries() {
		this.boundaries = new int[] { (int) (getXPositionPixel() - 0.5 * Math.abs(getHorizontalSpeedPixels())),
				getXPositionPixel() };
	}
				...
				if (getSecondsToLive() >= timeStep) {
					if (getOrientation() == -1) {
						if (getXPositionActual()
								- Math.abs(getHorizontalSpeedMeters()) * timeStep < (double) (getBoundaries()[0])
										/ 100) {
							double newPosX = getXPositionActual() - Math.abs(getHorizontalSpeedMeters()) * timeStep;
							double actualPosX = (double) getBoundaries()[0] / 100
						...
						if (getXPositionActual()
							...
							double newPosX = getXPositionActual() + Math.abs(getHorizontalSpeedMeters()) * timeStep;
							double actualPosX = (double) getBoundaries()[1] / 100
							...
							if (getOrientation() == 1)
								setXPositionActual(
										getXPositionActual() + Math.abs(getHorizontalSpeedMeters()) * timeStep);
							else
								setXPositionActual(
										getXPositionActual() - Math.abs(getHorizontalSpeedMeters()) * timeStep);
						}
