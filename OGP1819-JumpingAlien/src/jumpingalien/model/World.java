...
//	}
	...
	public boolean isValidVisibleWindowPosition(int[] visibleWindowPosition) {
		...
		if (visibleWindowPosition[0] < 0 || visibleWindowPosition[1] < 0)
			return false;
		if (visibleWindowPosition[0] + this.getVisibleWindowWidth() > this.getWorldSizeX()
				|| visibleWindowPosition[1] + this.getVisibleWindowHeight() > this.getWorldSizeY())
			return false;
		return true;
	}
	...
	public void setGeologicalFeature(int pixelX, int pixelY, int geologicalFeature) {
		...
		if (isValidPixelYPosition(pixelY) && isValidPixelXPosition(pixelX)) {
			this.tiles.remove(pixelX / getTileLength() + " " + pixelY / getTileLength() + " "
...
//			this.tiles.add(new int[] {pixelX/getTileLength(),pixelY/getTileLength(),geologicalFeature});
	...
	public void addGameObject(GameObject gameObject) throws RuntimeException {
		...
		if (this.getGeologicalFeature(gameObject.getXPositionPixel(), gameObject.getYPositionPixel()) == SOLID_GROUND
				&& this.getGeologicalFeature(gameObject.getXPositionPixel() + 1,
						gameObject.getYPositionPixel() + 1) != AIR)
			throw new RuntimeException();
	...
	public double getTimeStep(GameObject gameObject, double deltaT) {
		double velocityRoot = Math.sqrt(
				Math.pow(gameObject.getHorizontalSpeedMeters(), 2) + Math.pow(gameObject.getVerticalSpeedMeters(), 2));
		double accelerationRoot = Math.sqrt(Math.pow(gameObject.getHorizontalAcceleration(), 2)
		...
		if (getPlayer() == null) {
			...
			if ((this.getVisibleWindowWidth() - getPlayer().getXsize()) / 2 > 200
				...
				if (getPlayer().getXPositionPixel() <= 200) {
					newWindowXPos = 0;
				} else if (this.getWorldSizeX() - getPlayer().getXPositionPixel() - getPlayer().getXsize() <= 200) {
					...
					while (newWindowXPos + this.getVisibleWindowWidth() - getPlayer().getXPositionPixel()
				...
				if (getPlayer().getYPositionPixel() <= 200) {
					newWindowYPos = 0;
				} else if (this.getWorldSizeY() - getPlayer().getYPositionPixel() - getPlayer().getYsize() <= 200) {
					...
					while (newWindowYPos + this.getVisibleWindowHeight() - getPlayer().getYPositionPixel()
