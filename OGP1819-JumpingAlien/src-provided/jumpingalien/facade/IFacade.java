 ...
 * is provided to you stays in the <code>src-provided</code> folder. Note:
 * classes that belong to some package can be spread over multiple source
 ...
 * of <code>getActualPosition</code> should call one or more methods on the
 * given <code>alien</code> to retrieve its current location.</li>
 * 
 ...
 * a precondition or (2) if the method of your throws an exception (if so, wrap
 ...
 * representing aliens should follow the rules described in the
 * assignment.</b></li>
 * 
...
public interface IFacade {
    ...
    /**
     * Return a boolean indicating whether the code at stake is the effort of a team
     * of two students or the effort of an individual student.
     */
     ...
     * new Mazub will be stationary.
     */
    Mazub createMazub(int pixelLeftX, int pixelBottomY, Sprite... sprites) throws ModelException;

    /**
     * Return the actual position of the given alien. Returns an array of 2 doubles
     * {x, y} that represents the coordinates of the bottom-left corner of the given
     * alien in the world. Coordinates are expressed in meters.
     */
    ...
    /**
     * Return the pixel position of the given alien. Returns an array of 2 integers
     * {x, y} that represents the coordinates of the pixel in the game world
     * occupied by the bottom-left pixel of the given alien.
     */
    int[] getPixelPosition(Mazub alien) throws ModelException;

    /**
     * Return the orientation of the given alien. The resulting value is negative if
     * the given alien is oriented to the left, 0 if the given alien is oriented to
     * the front and positive if the given alien is oriented to the right.
     */
    int getOrientation(Mazub alien) throws ModelException;

    /**
     * Return the current velocity of the given alien. Returns an array consisting
     * of 2 doubles {vx, vy} that represents the horizontal and vertical components
     * of the given alien's current velocity in m/s.
     */
    double[] getVelocity(Mazub alien) throws ModelException;

    /**
     * Return the current acceleration of the given alien. Returns an array
     * consisting of 2 doubles {ax, ay} that represents the horizontal and vertical
     * components of the given alien's current acceleration in m/s^2.
     */
     ...
     * 
     * This method must only be implemented by teams of 2 students. Students working
     * on their own may ignore this method.
     */
     ...
     * directions, with given coordinate for the target tile, with given width and
     * height for the visible window, with its tiles filled with the given
     * geological features, and with no game objects yet. The geological features
     * are given in the order from left to right and (then) from bottom to top.
     * Tiles for which no geological feature is given, are filled with AIR.
     */
    World createWorld(int tileSize, int nbTilesX, int nbTilesY, int[] targetTileCoordinate, int visibleWindowWidth,
	    int visibleWindowHeight, int... geologicalFeatures) throws ModelException;
    ...
    /**
     * Return the size of the given game world in number of pixels. The resulting
     * array contains the width of the given world in pixels followed by the height
     * of the given world in pixels.
     */
    int[] getSizeInPixels(World world) throws ModelException;

    /**
     * Return the length of the side of a tile in the given world. The length is
     * expressed as a number of pixels.
     */
    int getTileLength(World world) throws ModelException;

    /**
     * Return the geological feature of the tile in the given world at the given
     * pixel position. The function returns 0 for AIR, 1 for SOLID GROUND, 2 for
     * WATER and 3 for MAGMA, 4 for ICE and 5 for GAS. Throws ModelException if
     * there is no tile at the given position. This method must return its result in
     * (nearly) constant time.
     */
    int getGeologicalFeature(World world, int pixelX, int pixelY) throws ModelException;

    /**
     * Set the geological feature of the tile in the given world at the given pixel
     * position to the given geological feature.
     */
    void setGeologicalFeature(World world, int pixelX, int pixelY, int geologicalFeature) throws ModelException;

    /**
     * Return the dimension of the visible window of the given world. The resulting
     * array contains the width of the visible window in pixels followed by the
     * height of the visible window in pixels.
     */
    int[] getVisibleWindowDimension(World world) throws ModelException;

    /**
     * Return the position of the bottom left corner of the visible window of the
     * given world.
     * 
     * This method must only be implemented by teams of 2 students. Students working
     * on their own may stick to the default implementation.
	...
	if (!isTeamSolution())
	    try {
		final int[] mazubPixelPosition = getPixelPosition(getMazub(world));
		return new int[] { Math.max(mazubPixelPosition[0] - 100, 0),
    ...
    /**
     * Check whether the given world has has the given object as one of its game
     * objects.
     */
    ...
    /**
     * Return the coordinate of the target tile in the given world. Returns an array
     * of 2 integers {x, y} that represents the tile coordinate of the target tile.
     */
    int[] getTargetTileCoordinate(World world) throws ModelException;

    /**
     * Set the coordinate of the target tile in the given world to the given tile
     * coordinate.
     */
    ...
    /**
     * Returns whether the game, played in the given game world, is over. The game
     * is over when either Mazub is terminated and removed from its world, or when
     * mazub has reached the target tile.
     */
    ...
    /**
     * Check whether the given game object is terminated. Terminated game objects
     * can not be located in a world.
     */
    boolean isTerminatedGameObject(Object gameObject) throws ModelException;

    /**
     * Check whether the given game object is dead. Dead game objects can still be
     * located in a world. After some period of time, dead game objects are
     * terminated.
     */
    boolean isDeadGameObject(Object gameObject) throws ModelException;

    /**
     * Return the actual position of the given game object. Returns an array of 2
     * doubles {x, y} that represents the coordinates of the bottom-left corner of
     * the given game object. Coordinates are expressed in meters.
     */
    ...
    /**
     * Return the pixel position of the given game object. Returns an array of 2
     * integers {x, y} that represents the coordinates of the pixel occupied by the
     * bottom-left pixel of the given game object.
     */
    int[] getPixelPosition(Object gameObject) throws ModelException;

    /**
     * Return the orientation of the given game object. The resulting value is -
     * negative if the given game object is oriented to the left or to the bottom, -
     * 0 if the given game object is oriented to the front, and - positive if the
     * given game object is oriented to the right or to the top.
     */
    int getOrientation(Object gameObject) throws ModelException;

    /**
     * Return the current velocity of the given game object. Returns an array
     * consisting of 2 doubles {vx, vy} that represents the horizontal and vertical
     * components of the given game object's current velocity in m/s.
     */
    ...
    /**
     * Return the world in which the given game object is positioned. Returns null
     * if the object is not positioned in a world.
     */
     ...
     */
    Slime createSlime(long id, int pixelLeftX, int pixelBottomY, School school, Sprite... sprites)
    ...
    /**
     * Return a boolean indicating whether the given slime belongs to the given
     * school.
     */
